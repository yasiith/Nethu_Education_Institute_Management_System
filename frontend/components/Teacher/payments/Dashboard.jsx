...
const ManagePaymentsPage = () => {
  ...
  const fetchMonthlyFees = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`https://nethu-education-institute-management.onrender.com/api/monthly-fees/${classId}`);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.put(`https://nethu-education-institute-management.onrender.com/api/monthly-fees/${classId}`, {
        month: selectedMonth,
        amount: newAmount,
      });
      setMonthlyFees(response.data.monthlyFees);
